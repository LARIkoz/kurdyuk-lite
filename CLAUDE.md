...
- **Codex (implementation reviewer)** — Claude запускает как **background sub-agent** (основная сессия продолжает работать):
  ...
  Agent(
    ...
    prompt="Run via Bash: codex exec 'Review plan.md as implementation expert: check feasibility, find pitfalls, compare with brief.md for scope drift. Be direct.' -s read-only -C .kurdyuk-lite/runs/<slug>/ -o /tmp/codex-plan-review.md. Then read the output and return findings."
  )
  ```
  Когда agent завершится — Claude читает результат и применяет к plan.md.
...
| Research (web/PDF/актуальное) | **Gemini CLI** | **background agent** | Свежие данные из интернета, PDF, 2M контекст  |
...
| Annotation (implementation)   | **Codex CLI**  | **background agent** | Implementation review + brief check           |
...
| Post-Implement Review         | **Codex CLI**  | **background agent** | Code review перед финалом                     |
| Final Review                  | Opus           | inline               | Честный verdict, ловить пропущенное           |

Думающие фазы → Opus. Кодящая фаза → Sonnet. Implementation checks → Codex. Web/PDF research → Gemini.

**Background agent = default для Gemini и Codex.** Запускать через `Agent(run_in_background=true)` — основная сессия продолжает работать. Блокирующий Bash только когда результат нужен прямо сейчас для следующего шага. Детали запуска — в skills `gemini-cli` и `codex-cli`.

**Параллельный запуск:** Gemini (research) + Codex (review) можно запустить одновременно как два background agent в одном сообщении.

**Когда вызывать Gemini в Research фазе:**

- Нужны свежие данные из интернета (новее августа 2025)
- Есть PDF документ для анализа или конвертации
- Контекст > 100K токенов (вся кодовая база, большой документ)
- Нужен скриншот / схема / wireframe анализ
- Kurdyuk Research phase: Gemini → raw material (background), Opus → синтез и решения (inline)
