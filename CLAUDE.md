...
1. **Pass 1 (broad scan)** — Gemini background: широкий поиск по теме, конкуренты, API, ограничения.
2. Opus синтезирует результат Pass 1 → фиксирует в `research.md`. Помечает пункты как `verified` или `unverified / needs deeper look`.
3. **Pass 2 (targeted, условный)** — если после Pass 1 есть `unverified` пункты, конфликтующие данные или вскрылась неожиданная область — Gemini background с конкретными вопросами по gaps. Если Pass 1 достаточен — сразу в Plan.
