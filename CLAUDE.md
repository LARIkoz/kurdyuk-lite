...
- **Design Reviewer (Gemini)** — visual review скриншотов, mockups, UI-решений. Запускается когда в задаче есть UI-компонент:
  ```
  Bash(
    command="source ~/.claude/keys.env && GEMINI_API_KEY=$GEMINI_API_KEY gemini -p 'Review these UI designs/screenshots. Check: visual consistency, spacing, contrast, accessibility, iOS/Android guidelines compliance. Compare with brief.md requirements. Be specific about issues.' -m gemini-2.5-pro -o text .kurdyuk-lite/runs/<slug>/ > /tmp/gemini-design-review.md 2>/dev/null",
    run_in_background=true
  )
...
```
Bash(
  command="source ~/.claude/keys.env && GEMINI_API_KEY=$GEMINI_API_KEY gemini -p 'Compare the implemented UI screenshot with the original mockup/design requirements. List: pixel differences, missing elements, spacing issues, color mismatches. Be specific.' -m gemini-2.5-pro > /tmp/gemini-visual-qa.md 2>/dev/null",
  run_in_background=true
)
